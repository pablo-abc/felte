...
If a validation library you use is not supported, feel free to [open an issue](https://github.com/pablo-abc/felte/issues), we would like to support the most used ones! You may also [build your own validation package](/docs/element/extending-felte).
...
const felteForm = document.querySelector('felte-form') as HTMLFelteFormElement<yup.InferType<typeof schema>>;
```
...
const felteForm = document.querySelector('felte-form') as HTMLFelteFormElement<z.infer<typeof schema>>;
```
...
const felteForm = document.querySelector('felte-form') as HTMLFelteFormElement<Infer<typeof schema>>;
```
